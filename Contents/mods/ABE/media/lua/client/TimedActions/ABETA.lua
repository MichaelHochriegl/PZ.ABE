...
ABETA.worldCraftingFinishedOld = function(object, recipe, character, retVal) 
	...
	elseif md.resultClass == "ISDoubleTileFurniture" then
		...
		local numberTable = ABETA.convertTableToNumberedTable(images)
		print("~ converted table unpacked ~")
		print(unpack(numberTable))
		retVal.object = ISDoubleTileFurniture:new(md.name, unpack(numberTable))
...
ABETA.worldCraftingFinished = function(object, recipe, character, retVal)
	if retVal.object then return end; -- might be overriden by another event, so we exit

	local md = object:getModData()["recipe"];
	local abe = object:getModData()["abe"];
	local images = ABE.getImages(getSpecificPlayer(character), recipe);
	local args = { };

	if (md.resultClassConstructorProps) then

		for _,pn in pairs(md.resultClassConstructorProps) do
			print("~ here in custom constructor args ~");
			print(pn);
			print(abe[pn])
			table.insert(args, abe[pn]);
		end
	else
		args = ABE.convertTableToNumberedTable(images);
	end

	print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
	print("resultClassConstructor")
	print(md.resultClassConstructor)
	print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
	retVal.object = md.resultClassConstructor(unpack(args));
...
local md = crafTec:getModData()["recipe"];
	...
	print("~ createRealObjectFromCrafTec ~")
	print("o")
	print(o)
...
function ABETA:isValid(square, north) 
	...
	if not self.haveAllTools then
		self.character:Say(getText("IGUI_ABE_PlayerSay_NoReqTools"));
		self:stop();
		return false;
	end
	...
	if bcUtils.tableIsEmpty(self.canProgress) then
		if not self:checkIfFinished() then
			if self.maxPartsProgress < 100 then
				self.character:Say(getText("IGUI_ABE_PlayerSay_NoPartsLeft"));
			else
				if self.isDeconstruction then
					return true;
				else
					self.character:Say(getText("IGUI_ABE_PlayerSay_NoSkill"));
				end
			end
			self:stop();
			return false;
				...
				if skill.progress < (skill.time / (100 / self.maxPartsProgress)) then
					...
					if self.recipe.use then
						for item,amount in pairs(self.recipe.use) do
							local useAmount = math.floor(skill.time / amount);
							local moduloUse = (skill.progress % useAmount);
...
function ABETA:checkIfFinished() 
	...
	local occupiedSquares = self.object:getModData()["occupiedSquares"]
	for sq, thump in pairs(occupiedSquares) do
		print(sq)
		print(thump)
		if isClient() then
...
function ABETA:use(item)
	...
	if not playerItem then
		self.character:Say("NOT FOUND");
		return false;
	end

	playerItem:Use();
	-- for item,amount in pairs(self.recipe.usesLeft) do
	-- 	local playerItem = self.character:getInventory:FindAndReturn(item);
	-- 	if playerItem then
	-- 		for i=1,amount do
	-- 			playerItem:Use();
...
function ABETA:new(character, object, isDeconstruction) 
	...
	o.knowsGameRecipe = true;
	if o.recipe.gameRecipe then
		o.knowsGameRecipe = o.character:getKnownRecipes():contains(o.recipe.gameRecipe);
	end
	...
	for profession,skills in pairs(o.recipe.requirements) do
		...
		elseif (profession == prof) or (profession == "any") then
			for k,skill in pairs(skills) do
				skill.name = k;
				print("~~~here in ABETA:new~~~")
				print(skill.progress)
				print(skill.time)
				print("skill.progress < skill.time")
				print(tostring(skill.progress < skill.time))
				print("skill.progress < skill.time / (100 / o.maxPartsProgress)")
				print(tostring(skill.progress < skill.time / (100 / o.maxPartsProgress)))
				print("o.character:getPerkLevel  " .. k)
				print(o.character:getPerkLevel(Perks.FromString(k)))
				print(skill.level)
...
Events.OnWorldCraftingFinished.Add(ABETA.worldCraftingFinished);
Events.OnWorldCraftingObjectCreated.Add(ABETA.worldCraftingObjectCreated);
